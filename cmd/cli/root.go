...
func initRoot(rootCmd *cobra.Command) {
	...
	rootCmd.PersistentFlags().IntVar(&hostConfig.RecursiveExtractMaxDepth, "recursive-extract-max-depth", config.DefaultRecursiveExtractMaxDepth, "Maximum depth for recursive archive extraction (default: 10)")
	rootCmd.PersistentFlags().StringVar(&hostConfig.RecursiveExtractMaxSize, "recursive-extract-max-size", config.DefaultRecursiveExtractMaxSize, "Maximum total size for recursive archive extraction (default: 5GB)")
	rootCmd.PersistentFlags().IntVar(&hostConfig.RecursiveExtractMaxFiles, "recursive-extract-max-files", config.DefaultRecursiveExtractMaxFiles, "Maximum total number of files for recursive archive extraction (default: 10000)")
