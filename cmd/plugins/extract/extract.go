...
func handleSevenZipError(sevenZipErr error, stderr string) (symLinkFiles []string, err error) {
	...
	case stderr == "":
		Logger.Warn("error with sevenzip", slog.String("error", sevenZipErr.Error()))
	...
	default:
		...
		Logger.Error("7z unsupported error: %s, returning err: %s", stderr, err.Error())
	}
	return
}
...
func parse7ZFileListLine(line string) (content FileProperties, err error) {
	...
	if errParse == nil {
		dateTimeStr = dateTime.UTC().Format(timeFormat)
	}
...
func (ec *extractResult) update(filePath string, archivePath string) (err error) {
	...
	if err != nil {
		Logger.Error("unable to get stats on file %s: %s", filePath, err.Error())
		return
	}
	...
	ec.extractedFiles = append(ec.extractedFiles, ExtractedFile{
		...
		Size:                 int(fi.Size()),
		LastModificationDate: fi.ModTime().UTC().Format(timeFormat),
	})
